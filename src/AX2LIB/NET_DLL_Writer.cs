...
namespace AX2LIB
{
    ...
    public class NET_DLL_Writer
    {
        ...
        public void Create()
        {
            ...
            string namespaces = "using NVP.API.Nodes;\nusing System;\nusing System.Collections.Generic;" + line_sep;
            ...
            work_for_CLASS_PROTOTYPEs(proptotype.CLASSES, false);
            work_for_CLASS_PROTOTYPEs(proptotype.Enumerations, true);

            void work_for_CLASS_PROTOTYPEs(List<CLASS_PROTOTYPE> elements, bool is_enums)
            {
                foreach (CLASS_PROTOTYPE class_wrapper in elements)
                {
                    ...
                    if (class_wrapper.Members != null)
                    {
                        foreach (COMPONENT_PROTOTYPE class_element in class_wrapper.Members)
                        {
                            ...
                            if (class_element.Name[0] != '_')
                            {
                                ...
                                if (is_enums) element_instructions = "";
                                ...
                                else if (class_element.TYPE == NET_DLL_PROTOTYPE.NET_TYPE.TYPE_METHOD_SET || class_element.TYPE == NET_DLL_PROTOTYPE.NET_TYPE.TYPE_FIELD)
                                {
                                    ...
                                    class_element_name_edited = prefix + class_element_name_edited;
                                ...
                                else if (class_element.TYPE == NET_DLL_PROTOTYPE.NET_TYPE.TYPE_METHOD_PRIVATE_VOID)
                                {
                                    ...
                                    content_type = NVP_XML.NodeViewType.Modifier;
                                    class_element_name_edited = "HiddenField_" + class_element_name_edited;
                                    element_instructions += "return null;";
                                    //element_instructions = $"this._i.{class_element.Name}({arguments_names_string});";
                                }
                                else if (class_element.TYPE == NET_DLL_PROTOTYPE.NET_TYPE.TYPE_ENUM)
                                {
                                    class_element_name_edited = "enum_" + class_element_name_edited;
                                    element_instructions += $"return new NodeResult({RootNsName}.{class_wrapper.Name}.{class_element.Name});" + line_sep;
                                ...
                                cs_content.AppendLine(
                                    $"{tab}public class {class_element_name_edited} : INode" + line_sep +
                                    $"{tab}" + "{" + line_sep +
                                    $"" + $"{exec}" +
                                    $"{tab}" + "}");
